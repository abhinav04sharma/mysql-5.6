...
int Rows_log_event::handle_idempotent_and_ignored_errors(Relay_log_info const *rli, int *err)
{
  ...
  if (error)
  {
    ...
    {
      if (log_warnings > 1 && (idempotent_error || ignored_error))
...
*/
void key_dealloc_cb(uchar *key_buf)
{
  my_free(key_buf);
}
...
int Rows_log_event::do_apply_event(Relay_log_info const *rli)
{
  ...
  */
  ...
  DBUG_PRINT("debug", ("m_table: 0x%lx, m_table_id: %llu%s", (ulong) m_table,
                       m_table_id.id(), (table && master_had_triggers ?
                       " (master had triggers)" : "")));
...
{
  DBUG_ASSERT(m_table != NULL);
  const bool overwrite = (slave_exec_mode == SLAVE_EXEC_MODE_IDEMPOTENT ||
    rli->is_table_idempotent(m_table->s->table_name.str) ||
